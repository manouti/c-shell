...
@after { System.out.println(root.toString());  }
	: ( procedureDec | functionDec | statement)* {  System.out.println("HELLO"); 
	}
 	;
...
statement
	: assignmentStatement { System.out.println("assignment: " + $assignmentStatement.node.toString());
...
@init { int minuses = 0; }
	: ( PLUS | MINUS { minuses++; })* negation {
		...
		$node = new Node("" + sign + "" + $negation.node);
	}
	;
